# Kronos BTC Directional Forecasting Research

This repository packages an applied research extension of the open-source
[Kronos](https://github.com/shiyu-coder/Kronos) financial foundation model. It
focuses on BTC directional forecasting, horizon-specific fine-tuning, structured
pruning, evaluation, and a prototype deployment interface.

## Research highlights

- The saved daily evaluation reaches **62.67% directional accuracy** on 75
  out-of-sample rows through 2026-03-16, versus a **53.33% majority baseline**.
- A dense / 25% / 50% / 75% hourly pruning sweep found the **50% sparse model**
  to be the strongest hourly variant: **53.67% accuracy** versus a 50.73%
  baseline, while also reducing long-side prediction bias.
- The project includes a Flask + Plotly research dashboard, market-data
  ingestion, paper-trading support, and opt-in Binance execution paths.
- Full experiment notes, caveats, and supporting tables are available in
  [`reports/sgp2_extension_20260405/REPORT.md`](reports/sgp2_extension_20260405/REPORT.md).

Results are historical research observations, not guarantees of future
performance. Trading code is provided for research and education; live order
...
logs, and real environment files are intentionally excluded. Copy an included
`.env.example` to a local `.env` only on your own machine and never commit
exchange credentials. See [`SECURITY.md`](SECURITY.md).

## Repository map

- `finetune/`, `finetune_csv/` — training, pruning, and evaluation workflows
- `reports/` — reproducible summaries, figures, and experiment tables
- `trading/` — strategy and paper/live execution scaffolding
- `webui/` — Flask + Plotly experiment dashboard
- `marketiser/` — API and frontend prototype
- `model/`, `examples/`, `tests/` — upstream model code, usage, and checks

## Upstream project

The original Kronos README is preserved below for model architecture, setup,
citations, and upstream attribution.
