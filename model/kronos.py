...
class Kronos(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        token_dropout_p,
        learn_te,
        use_output_adapter=False,
        output_adapter_bias=True,
        output_adapter_residual=True,
    ):
        super().__init__()
        self.s1_bits = s1_bits
        self.s2_bits = s2_bits
        ...
        self.output_adapter_bias = output_adapter_bias
        self.output_adapter_residual = output_adapter_residual
        ...
        self.output_adapter = (
            nn.Linear(self.d_model, self.d_model, bias=self.output_adapter_bias)
            if self.use_output_adapter
            else None
        )
        self.apply(self._init_weights)
        if self.output_adapter is not None:
            self._init_output_adapter()
    ...
    def _init_output_adapter(self):
        if self.output_adapter is None:
            return
        with torch.no_grad():
            nn.init.eye_(self.output_adapter.weight)
            if self.output_adapter.bias is not None:
                nn.init.zeros_(self.output_adapter.bias)

    def _apply_output_adapter(self, x):
        if self.output_adapter is None:
            return x
        if self.output_adapter_residual:
            return x + self.output_adapter(x)
    ...
    def forward(self, s1_ids, s2_ids, stamp=None, padding_mask=None, use_teacher_forcing=False, s1_targets=None):
        ...
        x = self.norm(x)
        x = self._apply_output_adapter(x)
...
class KronosPredictor:
    ...
    def predict(
        ...
        if norm_stats is None:
            ...
            if not isinstance(norm_stats, (tuple, list)) or len(norm_stats) != 2:
                raise ValueError("norm_stats must be None or a tuple/list of (mean, std).")
            x_mean = np.asarray(norm_stats[0], dtype=np.float32)
            x_std = np.asarray(norm_stats[1], dtype=np.float32)
            if x_mean.shape[-1] != x.shape[-1] or x_std.shape[-1] != x.shape[-1]:
                raise ValueError(
                    f"norm_stats feature size mismatch: expected {x.shape[-1]} "
                    f"got mean={x_mean.shape} std={x_std.shape}"
                )

        x_scale = np.maximum(x_std, 1e-5)
