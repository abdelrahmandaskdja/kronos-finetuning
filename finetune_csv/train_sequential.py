...
from finetune_base_model import (
    _apply_predictor_dropout_overrides,
    create_dataloaders,
    setup_logging as setup_basemodel_logging,
    train_model,
)
    ...
    def train_basemodel_phase(self):
        ...
        if getattr(self.config, 'pre_trained_predictor', True):
            ...
            model = Kronos.from_pretrained(
                ...
                use_output_adapter=getattr(self.config, 'predictor_use_output_adapter', False),
                output_adapter_bias=getattr(self.config, 'predictor_output_adapter_bias', True),
                output_adapter_residual=getattr(self.config, 'predictor_output_adapter_residual', True),
            )
        else:
            if self.rank == 0:
        ...
        _apply_predictor_dropout_overrides(model, self.config, logger)
