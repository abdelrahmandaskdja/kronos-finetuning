...
class QlibDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # Normalize the full sample window using statistics from the historical
        # context only, so future rows do not leak into the scale.
        context_len = min(max(int(self.config.lookback_window), 1), x.shape[0])
